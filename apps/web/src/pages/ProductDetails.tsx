...
export const ProductDetails: React.FC = () => {
  ...
  const toggleWishlistMutation = useMutation({
    ...
    onError: (err: any) => {
      if (!user) toast.error('Please login to manage wishlist');
  ...
  if (error || !data)
    return (
      <div className="flex justify-center items-center min-h-[60vh]">
        <div className="text-red-500 p-6 bg-red-50 dark:bg-red-900/10 rounded-2xl border border-red-100 dark:border-red-900/30 text-lg font-medium">
          Product not found.
        </div>
      </div>
    );
  ...
  const handleAddToCart = () => {
    ...
    if (product.variants && product.variants.length > 0) {
      ...
      if (missing.length > 0) {
        toast.error(
          `Please select: ${missing.map((m: any) => m.name).join(', ')}`
        );
        return;
      }
    }
  ...
  return (
    <div className="space-y-16 mt-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20">
          ...
          {product.images && product.images.length > 0 && (
            <div className="grid grid-cols-4 gap-4">
              {product.images.slice(0, 4).map((img: string, idx: number) => (
                <div
                  key={idx}
                  className="aspect-square bg-white/60 backdrop-blur-md dark:bg-dark-800/60 rounded-2xl overflow-hidden border border-white/40 dark:border-dark-700/50 cursor-pointer hover:border-primary-500 hover:shadow-lg hover:-translate-y-1 transition-all"
                >
                  <img
                    src={img}
                    alt=""
        ...
        >
          <div>
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold dark:text-white leading-tight tracking-tight mb-4">
              {product.name}
            </h1>
              ...
              <div className="flex text-yellow-400">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    ...
                    size={18}
                    fill={
                      star <= (product.rating || 0) ? 'currentColor' : 'none'
                    }
                    className={
                  ...
                  <div className="flex flex-wrap gap-3">
                    {variant.options.map((option: any) => (
                      <button
                        key={option.name}
                        onClick={() =>
                          handleVariantSelect(variant.name, option.name)
                        }
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center border border-gray-200 dark:border-dark-600 rounded-xl bg-white dark:bg-dark-800 p-1">
                  <button
                    ...
                    className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-dark-700 text-gray-600 dark:text-gray-300 transition-colors"
                  >
                    -
                  </button>
      ...
      <div className="pt-16">
        <h2 className="text-3xl font-extrabold dark:text-white mb-8 tracking-tight">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white dark:bg-dark-800 p-8 rounded-3xl border border-gray-100 dark:border-dark-700/50 shadow-sm">
              <h4 className="font-bold text-lg dark:text-white mb-6">
                Write a Review
              </h4>
              <form
                className="space-y-4"
              ...
              >
                <select
                  name="rating"
                  className="w-full border border-gray-200 dark:border-dark-700 bg-gray-50 dark:bg-dark-900 rounded-xl py-3 px-4 text-sm dark:text-white outline-none focus:ring-2 focus:ring-primary-500/50 transition-shadow cursor-pointer"
                  required
                >
                ...
                <textarea
                  name="comment"
                  className="w-full border border-gray-200 dark:border-dark-700 bg-gray-50 dark:bg-dark-900 rounded-xl py-3 px-4 text-sm dark:text-white outline-none focus:ring-2 focus:ring-primary-500/50 transition-shadow h-32 resize-none"
            ...
            <div className="bg-white dark:bg-dark-800 p-8 rounded-3xl border border-gray-100 dark:border-dark-700/50 shadow-sm">
              ...
              >
                ...
                <Button
                  variant="outline"
                  type="submit"
                  className="rounded-xl px-8 border-gray-200 dark:border-dark-700 hover:bg-gray-50 dark:hover:bg-dark-700"
                >
                  Ask Question
                </Button>
              </form>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        {!data || data.length === 0 ? (
          <div className="text-gray-500 bg-gray-50 dark:bg-dark-800/50 p-6 rounded-2xl text-center">
          ...
          data.map((r: any) => (
            <div
              key={r._id}
              className="bg-white dark:bg-dark-800 p-6 rounded-2xl border border-gray-100 dark:border-dark-700/50 shadow-sm"
            >
...
const QAList = ({ productId }: { productId: string }) => {
  ...
  if (isLoading)
    return <div className="text-gray-500 animate-pulse">Loading Q&A...</div>;
              ...
              <div className="pl-6 border-l-2 border-gray-100 dark:border-dark-700">
                {q.answer ? (
                  <p className="text-gray-600 dark:text-gray-300 text-sm flex items-start">
                    <span className="font-bold text-gray-900 dark:text-gray-400 mr-2">
                      A:
                    </span>
                    <span>{q.answer}</span>
                  </p>
        ...
        {data.map((product: any, i: number) => (
          ...
          >
            <Link
              ...
              className="aspect-square bg-gray-50 dark:bg-dark-900 rounded-xl mb-4 overflow-hidden flex items-center justify-center"
            >
              <img
                src={product.thumbnail || FALLBACK_IMAGE}
                alt={product.name}
              ...
              <div className="mt-auto pt-2 flex justify-between items-center">
                ...
                <Link to={`/product/${product.slug}`}>
                  <Button
                    variant="outline"
                    size="sm"
                    className="rounded-lg h-8 w-8 p-0 flex items-center justify-center"
                  >
                    <ArrowRight size={14} />
                  </Button>
                </Link>
