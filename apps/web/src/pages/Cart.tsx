...
import { Button, EmptyState, useConfirm } from '@techverse/ui';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {cartItems.map((item) => (
              <div
                key={item.product}
                className="flex flex-col sm:flex-row items-center bg-white/70 backdrop-blur-md dark:bg-dark-800/70 p-5 rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] dark:shadow-[0_4px_20px_rgb(0,0,0,0.2)] border border-white/40 dark:border-dark-700/50 gap-6 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:hover:shadow-[0_8px_30px_rgb(0,0,0,0.3)] transition-all"
              >
                ...
                <div className="flex-1 text-center sm:text-left">
                  <Link
                    ...
                    className="font-bold text-lg text-gray-900 dark:text-white hover:text-primary-600 transition-colors"
                  >
                    {item.name}
                  </Link>
                  <p className="text-xl font-extrabold text-primary-600 dark:text-primary-400 mt-2">
                    ₹{item.price.toFixed(2)}
                  </p>
                </div>
                <div className="flex flex-col sm:items-end space-y-3">
                  <span className="text-sm font-medium bg-gray-100 dark:bg-dark-700 px-3 py-1 rounded-lg dark:text-gray-300">
                    Qty: {item.qty}
                  </span>
                  <button
          ...
          <div className="bg-white/80 backdrop-blur-xl dark:bg-dark-800/80 p-8 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] border border-white/40 dark:border-dark-700/50 h-fit sticky top-28">
            <h2 className="text-2xl font-bold dark:text-white mb-8 tracking-tight">
              Order Summary
            </h2>
            <div className="space-y-5 text-sm text-gray-600 dark:text-gray-400">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 px-2 py-1 rounded-md">
                  Free
                </span>
              </div>
              <div className="flex justify-between items-center">
