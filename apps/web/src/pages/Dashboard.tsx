...
export const Dashboard: React.FC = () => {
  ...
  if (!user)
    return (
      <div className="dark:text-white p-8">Please login to view dashboard.</div>
    );
          ...
          <div className="space-y-6">
            ...
            {ordersLoading ? (
              ...
              ordersData?.map((order: any) => (
                <div
                  key={order._id}
                  className="bg-white dark:bg-dark-800 rounded-xl border border-gray-100 dark:border-dark-700 overflow-hidden"
                >
                  <div className="bg-gray-50 dark:bg-dark-900 p-4 border-b border-gray-100 dark:border-dark-700 flex flex-wrap justify-between items-center text-sm gap-4">
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          window.open(
                            `/api/orders/${order._id}/invoice?token=${user.token}`,
                            '_blank'
                          )
                        }
                      >
                  ...
                  <div className="p-6 divide-y divide-gray-100 dark:divide-dark-700">
                    {order.orderItems.map((item: any, i: number) => (
                      <div
                        key={i}
                        className="flex items-center space-x-6 py-4 first:pt-0 last:pb-0"
                      >
            ...
            {wishlistLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {wishlistData?.map((item: any) => (
                  <div
                    key={item._id}
                    className="bg-white dark:bg-dark-800 rounded-xl p-4 border border-gray-100 dark:border-dark-700 flex flex-col group"
                  >
                    <Link
                      to={`/product/${item.slug}`}
                      className="aspect-square bg-gray-100 dark:bg-dark-700 rounded-lg mb-4 overflow-hidden"
                    >
                      {item.thumbnail ? (
                        <img
                          src={item.thumbnail}
                    ...
                    <div className="mt-auto">
                      <Link
                        to={`/product/${item.slug}`}
                        className="font-semibold text-gray-900 dark:text-white mb-2 hover:text-primary-600"
                      >
                        {item.name}
                      </Link>
              ...
              <div className="space-y-4">
                {enquiriesData?.map((enq: any) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-dark-900 p-3 rounded-lg mb-2">
                      {enq.message}
                    </p>
                    {enq.reply && (
