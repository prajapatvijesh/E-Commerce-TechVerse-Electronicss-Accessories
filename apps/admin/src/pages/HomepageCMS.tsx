...
export const HomepageCMS: React.FC = () => {
  ...
  useEffect(() => {
    if (settingsData) {
      if (settingsData.heroBanners?.length > 0)
        setHeroBanners(settingsData.heroBanners);
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) =>
      axios.put('/api/homepage-settings', data, {
  ...
  if (isLoading)
    return <div className="p-8 dark:text-white">Loading CMS settings...</div>;
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {heroBanners.map((banner, idx) => (
            <div
              key={idx}
              className="p-4 border border-gray-200 dark:border-dark-700 rounded-xl relative flex flex-col gap-4"
            >
              <button
