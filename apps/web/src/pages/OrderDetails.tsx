          ...
          {(order.trackingNumber || order.courierName) && (
            <div className="mb-8 p-4 bg-gray-50 dark:bg-dark-800 rounded-xl flex items-center space-x-6 border border-gray-200 dark:border-dark-700">
              ...
              <div>
                ...
                <p className="font-bold dark:text-white font-mono tracking-wider">
                    ...
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center border-4 ${
                        isCompleted
                          ...
                          : 'bg-white dark:bg-dark-800 border-gray-200 dark:border-dark-700 text-gray-400'
                      } ${isCurrent ? 'ring-4 ring-primary-100 ring-opacity-50' : ''}`}
                    >
                      {milestone.icon}
                    </div>
                    <p
                      className={`mt-3 font-semibold ${isCompleted ? 'text-gray-900 dark:text-white' : 'text-gray-400'}`}
                    >
                      {milestone.label}
                    </p>
                  </div>
                    ...
                    <div key={i} className="flex space-x-4">
                      <div className="flex flex-col items-center">
                        ...
                        {i !== order.trackingHistory.length - 1 && (
                          <div className="w-0.5 h-full bg-gray-200 dark:bg-dark-700 my-1"></div>
                        )}
                      </div>
                      <div>
          ...
          <CardContent>
            <div className="space-y-4">
              {order.orderItems.map((item: any) => (
                <div
                  key={item._id}
                  className="flex items-center space-x-4 border-b border-gray-100 dark:border-dark-700 pb-4 last:border-0"
                >
                  <img
                    src={item.image}
                    alt={item.name}
