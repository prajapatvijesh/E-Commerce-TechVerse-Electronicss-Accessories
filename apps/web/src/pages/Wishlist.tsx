    ...
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      {products.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-dark-800 rounded-2xl shadow-sm border border-gray-100 dark:border-dark-700">
          <h3 className="text-xl font-medium dark:text-gray-300 mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((product: any) => (
            <div
              key={product._id}
              className="bg-white dark:bg-dark-800 rounded-xl overflow-hidden shadow-sm border border-gray-100 dark:border-dark-700 group flex flex-col"
            >
              <Link
                to={`/product/${product._id}`}
                className="block relative overflow-hidden aspect-square bg-gray-100 dark:bg-dark-900"
              >
                <img
              ...
              <div className="p-4 flex flex-col flex-grow">
                <Link
                  to={`/product/${product._id}`}
                  className="text-lg font-semibold dark:text-white hover:text-primary-600 line-clamp-1 mb-2"
                >
                  {product.name}
                </Link>
