...
export const Compare: React.FC = () => {
  ...
  const allAttributes = Array.from(
    new Set(items.flatMap((item) => item.attributes.map((attr) => attr.name)))
  );
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] px-4">
        ...
        <h2 className="text-3xl font-extrabold dark:text-white tracking-tight mb-4 text-center">
          Your compare list is empty
        </h2>
        <p className="text-gray-500 text-center mb-8 max-w-md">
          Add products to your comparison list to see them side-by-side and find
        ...
        <Link to="/shop">
          <Button
            variant="primary"
            size="lg"
            className="rounded-xl px-8 shadow-lg shadow-primary-500/20 hover:shadow-primary-500/40"
          >
            Explore Products
          </Button>
        </Link>
    ...
    <div className="max-w-[1400px] mx-auto px-4 py-8 space-y-8 mt-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button
          ...
          className="text-red-500 hover:bg-red-50 hover:border-red-200 dark:hover:bg-red-900/20 border-gray-200 dark:border-dark-700 rounded-xl"
        >
          Clear All
        </Button>
      </div>
        ...
        <table className="w-full text-left min-w-[800px] border-collapse">
          <thead>
            <tr>
              ...
              {items.map((item) => (
                <th
                  key={item._id}
                  className="p-6 border-b border-gray-100 dark:border-dark-700/50 align-top relative min-w-[250px] w-1/4"
                >
                  <button
                  ...
                  <Link to={`/product/${item._id}`} className="block group">
                    ...
                    <h3 className="font-bold text-gray-900 dark:text-white text-lg leading-tight group-hover:text-primary-600 transition-colors line-clamp-2 mb-2">
                      {item.name}
                    </h3>
                  </Link>
                  <div className="mt-4">
                    ...
                    <Button
                      variant="primary"
                      className="w-full rounded-xl flex justify-center items-center space-x-2"
              ...
              {Array.from({ length: 4 - items.length }).map((_, i) => (
                <th
                  key={`empty-${i}`}
                  className="p-6 border-b border-gray-100 dark:border-dark-700/50 align-middle text-center w-1/4"
                >
                  <div
                    className="aspect-square rounded-2xl border-2 border-dashed border-gray-200 dark:border-dark-700 flex flex-col items-center justify-center text-gray-400 hover:text-primary-500 hover:border-primary-500 transition-colors cursor-pointer"
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-dark-700/50">
            <tr className="hover:bg-gray-50 dark:hover:bg-dark-900/30 transition-colors">
              <td className="p-4 px-6 bg-gray-50/50 dark:bg-dark-900/30 font-semibold text-gray-700 dark:text-gray-300 border-r border-gray-100 dark:border-dark-700/50 sticky left-0 z-10 backdrop-blur-sm">
                Rating
              </td>
              {items.map((item) => (
                <td
                  key={item._id}
                  className="p-4 px-6 text-gray-600 dark:text-gray-400"
                >
              ...
              {Array.from({ length: 4 - items.length }).map((_, i) => (
                <td key={`empty-rating-${i}`} className="p-4 px-6"></td>
              ))}
            </tr>
            ...
            {allAttributes.map((attrName) => (
              <tr
                key={attrName}
                className="hover:bg-gray-50 dark:hover:bg-dark-900/30 transition-colors"
              >
                <td className="p-4 px-6 bg-gray-50/50 dark:bg-dark-900/30 font-semibold text-gray-700 dark:text-gray-300 border-r border-gray-100 dark:border-dark-700/50 sticky left-0 z-10 backdrop-blur-sm capitalize">
                  {attrName}
                </td>
                {items.map((item) => {
                  const attrValue = item.attributes.find(
                ...
                {Array.from({ length: 4 - items.length }).map((_, i) => (
                  <td key={`empty-${attrName}-${i}`} className="p-4 px-6"></td>
                ))}
              </tr>
            ))}
          </tbody>
